...
void display(){
	...
	mat4 view = look_at(vec3(0.0, 0.0, -100.0), vec3(lookAtX, lookAtY, lookAtZ), vec3(0.0, 1.0, 0.0));
	...
	// bottom-right
	spinOffset += 1.0;
	if (spinOffset >= 360.0) spinOffset = 0.0;

	zOffset += 0.2;
	if (zOffset >= 100.0) zOffset = 0.0;
	view = translate(identity_mat4(), vec3(0.0, 0.0, -40.0));
	persp_proj = ortho(-30.0, 30.0, -30.0, 30.0, 0.1, (100.0 - zOffset));
	model = rotate_x_deg(identity_mat4(), 45);
	model = rotate_y_deg(model, spinOffset);
	...
	// top-left
	view = translate(identity_mat4(), vec3(0.0, 0.0, -40.0));
	persp_proj = perspective(45.0, ((float)width / (float)height) + (zOffset/10), 0.1, 100.0);
	model = rotate_y_deg(identity_mat4(), 90);
	...
	// top-right
	view = translate(identity_mat4(), vec3(0.0, 0.0, -40.0));
	persp_proj = perspective((45.0 + zOffset), (float)width / (float)height, 0.1, 100.0);
